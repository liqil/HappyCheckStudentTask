...
public class ZzExcelCreator implements ExcelManager {
    ...
    @Override
    public ZzExcelCreator createExcel(String pathDir, String name) throws IOException, BiffException  {
        File dir = new File(pathDir);
        if (!dir.exists())
            dir.mkdirs();

        File file = new File(pathDir + name + ".xls");
        if (!file.exists())
            writableWorkbook = Workbook.createWorkbook(file);
        else {
            ///< file exist, so open it
            openExcel(file);
        }
        return this;
    }
    ...
    public ZzExcelCreator openExcel(File file) throws IOException, BiffException {
        ...
        Log.i("liqil","shit openExcel"+ wb.toString());
    ...
    public ZzExcelCreator createSheet(String name) {
        ...
        if(writableWorkbook.getSheet(name) == null) {
            writableSheet = writableWorkbook.createSheet(name, writableWorkbook.getNumberOfSheets());
        } else {
            openSheet(name);
        }
    ...
    @Override
    public ZzExcelCreator openSheet(String name){
        checkNullFirst();
        Log.e("xxxx", writableWorkbook.getNumberOfSheets() + "");
        writableSheet = writableWorkbook.getSheet(name);
        return this;
    }
